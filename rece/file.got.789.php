...
$CONFIG = '{"lang":"en","error_reporting":true,"show_hidden":false,"hide_Cols":true,"calc_folder":true}';
...
$auth_users = array(
    '鱼鱼' => '$2y$10$YMfpzSNG0PQyFBJJhNeffu4WSE54z0S7USq77gcabdJzh3dN1TZKy', //admin@123
);
...
$root_path = $_SERVER['DOCUMENT_ROOT'].'/abusb320';
    ...
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
        ...
        if(function_exists('password_verify')) {
            if (isset($auth_users[$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']])) {
                ...
                unset($_SESSION[FM_SESSION_ID]['logged']);
                fm_set_msg('登录失败', 'error');
                fm_redirect(FM_SELF_URL);
            }
        } else {
            fm_set_msg('密码校验失败，请升级你的PHP版本', 'error');;
        }
    } else {
        // Form
        unset($_SESSION[FM_SESSION_ID]['logged']);
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? '<b>%s</b> 已删除' : 'File <b>%s</b> deleted';
            fm_set_msg(sprintf($msg, fm_enc($del)));
        } else {
            $msg = $is_dir ? '<b>%s</b> 未删除' : 'File <b>%s</b> not deleted';
    ...
    if (fm_isvalid_filename($new) && $new != '' && $new != '..' && $new != '.') {
        ...
        if ($_GET['type'] == "file") {
            ...
            if (fm_mkdir($path . '/' . $new, false) === true) {
                fm_set_msg(sprintf('<b>%s</b> 创建', $new));
            } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
                fm_set_msg(sprintf('<b>%s</b> 已经存在，请换个新名字吧', fm_enc($new)), 'alert');
            } else {
                fm_set_msg(sprintf('<b>%s</b> 创建失败', fm_enc($new)), 'error');
            }
        }
    } else {
...
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish']) && !FM_READONLY) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            $msg = $move ? '所选文件已经移动成功' : '所选文件已经复制成功';
            fm_set_msg($msg);
        } else {
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete')?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('确定删除该文件夹吗？');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-9">
            <ul class="list-inline footer-action">
                ...
                <li class="list-inline-item"><input type="submit" class="hidden" name="delete" id="a-delete" value="Delete" onclick="return confirm('删除选择的文件及文件夹？')">
                    <a href="javascript:document.getElementById('a-delete').click();" class="btn btn-small btn-outline-primary btn-2"><i class="fa fa-trash"></i> <?php echo lng('Delete') ?> </a></li>
                <li class="list-inline-item"><input type="submit" class="hidden" name="zip" id="a-zip" value="zip" onclick="return confirm('压缩选择的文件及文件夹为zip格式？')">
                    <a href="javascript:document.getElementById('a-zip').click();" class="btn btn-small btn-outline-primary btn-2"><i class="fa fa-file-archive-o"></i> <?php echo lng('Zip') ?> </a></li>
                <li class="list-inline-item"><input type="submit" class="hidden" name="tar" id="a-tar" value="tar" onclick="return confirm('压缩选择的文件及文件夹为tar格式？')">
...
<head>
    ...
    <style>
        ...
        * { -webkit-border-radius:0 !important;-moz-border-radius:0 !important;border-radius:0 !important; }
        ...
        .fa.fa-home { font-size:1.3em;color:#ff0073;vertical-align:bottom  }
        ...
        .inline-actions > a > i { font-size:1.5em;margin-left:5px;color:#dc3545;padding:3px;border-radius:3px  }
...
function lng($txt) {
    ...
    // English Language
    $tr['en']['AppName']        = '文件管理器';      $tr['en']['AppTitle']           = '文件管理器';
    $tr['en']['Login']          = '登录';                $tr['en']['Username']           = '用户名';
    $tr['en']['Password']       = '密码';               $tr['en']['Logout']             = '注销';
    $tr['en']['Move']           = '移动';                   $tr['en']['Copy']               = '复制';
    $tr['en']['Save']           = '保存';                   $tr['en']['SelectAll']          = '全选';
    $tr['en']['UnSelectAll']    = '全部取消';           $tr['en']['File']               = '文件';
    $tr['en']['Back']           = '返回';                   $tr['en']['Size']               = '大小';
    $tr['en']['Perms']          = '权限';                  $tr['en']['Modified']           = '日期';
    $tr['en']['Owner']          = '所有者';                  $tr['en']['Search']             = '搜索';
    $tr['en']['NewItem']        = '新建';               $tr['en']['Folder']             = '文件夹';
    $tr['en']['Delete']         = '删除';                 $tr['en']['Rename']             = '重命名';
    $tr['en']['CopyTo']         = '复制到';                $tr['en']['DirectLink']         = '直连地址';
    $tr['en']['UploadingFiles'] = '上传一堆文件';           $tr['en']['ChangePermissions']  = '修改权限';
    $tr['en']['Copying']        = '复制中';                $tr['en']['CreateNewItem']      = '新建菜单';
    $tr['en']['Name']           = '名称';                   $tr['en']['AdvancedEditor']     = '高级编辑器';
    $tr['en']['RememberMe']     = '记住我';            $tr['en']['Actions']            = '操作';
    $tr['en']['Upload']         = '上传';                 $tr['en']['Cancel']             = '撤销';
    $tr['en']['InvertSelection']= '反选';       $tr['en']['DestinationFolder']  = '新路径';
    $tr['en']['ItemType']       = '类型';              $tr['en']['ItemName']           = '名称';
    $tr['en']['CreateNow']      = '创建';             $tr['en']['Download']           = '下载';
    $tr['en']['Open']           = '打开';                   $tr['en']['UnZip']              = '解压';
    $tr['en']['UnZipToFolder']  = '解压到文件夹';        $tr['en']['Edit']               = '编辑';
    $tr['en']['NormalEditor']   = '普通编辑器';          $tr['en']['BackUp']             = '备份';
    $tr['en']['SourceFolder']   = '原路径';          $tr['en']['Files']              = '文件';
    $tr['en']['Move']           = '移-动';                   $tr['en']['Change']             = '修改';
    $tr['en']['Settings']       = '设置';               $tr['en']['Language']           = '语言';
    $tr['en']['MemoryUsed']     = '内存使用';            $tr['en']['PartitionSize']      = '硬盘总体使用';
    $tr['en']['ErrorReporting'] = '错误反馈';        $tr['en']['ShowHiddenFiles']    = '显示隐藏文件/夹';
    $tr['en']['Full size']      = '已使用';              $tr['en']['Help']               = '帮助';
    $tr['en']['Free of']        = '空闲';                $tr['en']['Preview']            = '预览';
    $tr['en']['Help Documents'] = '帮助文档';         $tr['en']['Report Issue']       = '错误.反馈';
    $tr['en']['Generate']       = '生成';               $tr['en']['FullSize']           = '当前目录大小';
    $tr['en']['FreeOf']         = '空.闲';                $tr['en']['CalculateFolderSize']= '显示文件大小';
    $tr['en']['ProcessID']      = 'Process ID';
    $tr['en']['HideColumns']    = '隐藏权限和所有者';
    $tr['en']['Check Latest Version']= '升级检查'; $tr['en']['Generate new password hash'] = '生成新的hash密码';
